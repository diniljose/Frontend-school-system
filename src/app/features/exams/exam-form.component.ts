...
export class ExamFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.api.get(`/exams/${id}`).subscribe({
        next: (res: any) => this.exam = { ...(res.data || res) },
  ...
  onSubmit(): void {
    ...
    const payload = { ...this.exam };
    // If classId is set but classes array isn't, put classId into classes array
    if (payload.classId && !payload.classes?.length) {
      payload.classes = [payload.classId];
    }
    const obs = this.isEdit()
      ? this.api.patch(`/exams/${this.exam._id}`, payload)
    ...
    obs.subscribe({
      ...
      error: (err) => { this.saving.set(false); this.toast.error(err?.error?.message?.join?.(', ') || 'Failed to save exam'); }
    });
  }
}
